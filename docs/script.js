...
class NavigationHighlight {
    ...
    init() {
        ...
        this.sections = Array.from(document.querySelectorAll('section[id]'))
            .filter(section => document.querySelector(`#navigation-menu a[href="#${section.id}"]`));
        
        if (this.sections.length > 0 && this.navLinks.length > 0) {
            this.setInitialActiveState();

            // Keep the active link aligned with the section currently in view.
            window.addEventListener('scroll', () => this.updateActiveSection(), { passive: true });
            
            // Handle hash changes from navigation and browser history.
    ...
    updateActiveSection() {
        ...
        if (activeSection) {
            this.highlightNavLink(activeSection.id);
        }
    }
}
...
class MarkdownLoader {
    constructor() {
        this.sections = ['about', 'news', 'publications', 'resources', 'resume', 'talks'];
        this.init();
    }
